...
void get_object_by_signed_url()
{
    ...
    signed_url = cos_gen_signed_url(options, &bucket, &object, 
    ...
    s = cos_get_object_to_buffer_by_url(options, &url, 
            headers, params, &buffer, &resp_headers);
...
void get_cos_dir_to_local_dir()
{
    ...
    options = cos_request_options_create(parent_pool);
    init_sample_request_options(options, is_cname);
    aos_str_set(&bucket, BUCKET_NAME);
    ...
    while (params->truncated) {
        ...
        aos_list_for_each_entry(cos_list_object_content_t, list_content, &params->object_list, node) {
            if ('/' == list_content->key.data[strlen(list_content->key.data) - 1]) {
                ...
                s = cos_get_object_to_file(options, &bucket, &object, 
