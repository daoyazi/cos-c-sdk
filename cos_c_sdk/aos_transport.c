...
char* aos_read_body_memory(aos_http_request_t *req)
{
    ...
    int bytes = 0;
    while (1) {
        bytes = aos_read_http_body_memory(req, buffer, buffer_len);
        if (bytes==0)
        {
            return data;
        }
...
int aos_curl_http_transport_perform(aos_http_transport_t *t_)
{
    ...
    // if(HTTP_POST == t->req->method) {
    //     char *send_data = aos_read_body_memory(t->req);
    //     curl_easy_setopt(t->curl, CURLOPT_POSTFIELDS, send_data);
    //     free(send_data);        
    // }
