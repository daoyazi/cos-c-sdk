            ...
            )    /* caller digest to be filled in */
{
    ...
    unsigned char tk2[SHA256_BLOCK_SIZE];
    unsigned char *bufferIn = malloc(text_len + 1024);
    unsigned char *bufferOut = malloc(text_len + 1024);
    int           i;
     ...
     */
    memset( bufferIn, 0x00, text_len+64 );
     ...
     */
    memset( bufferOut, 0x00, text_len+64 );
    memcpy( bufferOut, k_opad, 64 );
    memcpy( bufferOut + 64, tk2, SHA256_BLOCK_SIZE );

    SHA256( bufferOut, 64 + SHA256_BLOCK_SIZE, digest);
    free(bufferIn);
    free(bufferOut);
}
